...
export default async function ProjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    ...
    <div className="grid gap-5 xl:grid-cols-[1fr_2fr]">
      ...
      <section><h2 className="mb-3 font-display text-lg uppercase text-white">Задачи проекта</h2><div className="grid gap-3 md:grid-cols-2">{tasks.filter((task) => task.project?.id === project.id).map((task) => <TaskCard key={task.id} task={task} />)}</div></section>
    </div>
  </>;
}
