...
export default async function BattleSheetPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="mx-auto max-w-5xl">
    ...
    <header className="mb-7 border-b border-white/10 pb-7"><p className="eyebrow flex items-center gap-2"><ShieldAlert size={13} />Боевой лист</p><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><h1 className="font-display text-4xl uppercase text-white sm:text-5xl">{event.title}</h1><p className="mt-3 flex items-center gap-2 text-sm text-zinc-500"><MapPin size={14} />{event.city} · {event.venue}</p></div><StatusBadge status={event.status} /></div></header>
