...
export function SectionPage({ eyebrow, title, description, rows, action }: { eyebrow: string; title: string; description: string; rows: Row[]; action?: React.ReactNode }) {
  ...
  return <>
    <PageHeader eyebrow={text(eyebrow)} title={text(title)} description={text(description)} action={action} />
    <div className="table-shell">
      ...
      {!rows.length && <div className="p-12 text-center text-sm text-zinc-600">{t("common.noData")}</div>}
    </div>
  </>;
}
