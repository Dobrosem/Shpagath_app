...
const navigation = [
  ...
  { href: "/events", label: "Концерты", icon: CalendarDays },
  { href: "/rehearsals", label: "Репетиции", icon: UsersRound },
  { href: "/promo", label: "Промо", icon: Megaphone },
...
function Mark() {
  return <Link href="/dashboard" className="flex items-center gap-3">
    ...
    <div><p className="font-display text-base font-semibold uppercase tracking-[.22em] text-zinc-100">Saphath</p><p className="text-[8px] uppercase tracking-[.28em] text-zinc-600">Workspace</p></div>
  </Link>;
}
  ...
  return <div className="min-h-screen bg-void text-zinc-300">
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-60 border-r border-white/[.06] bg-[#090a0a] px-4 py-6 lg:flex lg:flex-col">
      ...
      <div className="mt-auto space-y-1 border-t border-white/[.06] pt-4">
        <Link href="/settings" className="nav-item"><Settings size={17} />Настройки</Link>
        <button onClick={signOut} className="nav-item w-full"><LogOut size={17} />Выйти</button>
      </div>
    </aside>
    ...
    <div className="lg:pl-60">
      <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-white/[.06] bg-void/85 px-4 backdrop-blur-xl sm:px-7">
        <button className="lg:hidden" onClick={() => setOpen(true)} aria-label="Открыть меню"><Menu size={21} /></button>
        <div className="hidden items-center gap-2 text-[10px] uppercase tracking-[.15em] text-zinc-700 lg:flex"><SlidersHorizontal size={13} /> Внутренний портал</div>
        <div className="flex items-center gap-3">
          <div className="hidden text-right sm:block"><p className="text-xs text-zinc-300">{profile.full_name}</p><p className="text-[9px] uppercase tracking-wider text-zinc-600">{profile.role}</p></div>
    ...
    <nav className="fixed inset-x-0 bottom-0 z-40 flex h-16 items-center justify-around border-t border-white/10 bg-[#090a0a]/95 px-2 backdrop-blur-xl lg:hidden">
      {navigation.slice(0, 5).map(({ href, label, icon: Icon }) => <Link key={href} href={href} className={cn("flex flex-col items-center gap-1 px-3 text-[9px] text-zinc-600", pathname.startsWith(href) && "text-zinc-100")}><Icon size={18} /><span>{label}</span></Link>)}
    </nav>
  </div>;
}
